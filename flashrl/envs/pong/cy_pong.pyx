...
cdef class CyPong:
    ...
    def __init__(self, char[:, :, :] obs, unsigned char[:] acts, float[:] rewards, float[:] dones, size_t n,
                 int size_x, int size_y, float max_dy):
        self.envs = <Pong*>calloc(n // 2, sizeof(Pong))
        self.n = n
        cdef int i
        for i in range(0, self.n, 2):
            self.envs[i//2] = Pong(&obs[i, 0, 0], &obs[i+1, 0, 0], &acts[i], &acts[i+1], &rewards[i], &rewards[i+1],
                                   &dones[i], &dones[i+1], size_x=size_x, size_y=size_y, max_dy=max_dy, t=0,
                                   paddle0_x=0, paddle0_y=0, paddle1_x=0, paddle1_y=0, x=0, y=0, dx=0, dy=0)
