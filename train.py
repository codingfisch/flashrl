...
env = frl.envs.Pong(n_agents=2**14)  # just replace Pong with Grid to try (for MultiGrid use commented code block below)
learn = frl.Learner(env=env.reset(SEED))
#learn.model = frl.LSTMPolicy(learn.env, hidden_size=64).to(learn.device, learn.dtype)  # uncomment for smaller model
